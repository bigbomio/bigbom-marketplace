...
class JobDetailBid extends Component {
    ...
    getReasonPaymentRejected = async paymentRejectReason => {
        const reason = await api.getReason(Number(paymentRejectReason.reason)).text;
        if (this.mounted) {
                                        ...
                                        <Grid container>
                                            ...
                                            {disputeStt.started &&
                                                (disputeStt.clientResponseDuration > Date.now() ? (
                                                    ...
                                                    (clientRespondedDispute.commitDuration > Date.now() && (
                                                        <Countdown
                                                            onReload
                                            ...
                                            {jobData.status.reject && rejectPaymentDuration && (
                                                <Countdown onReload name="Respone duration" expiredTime={rejectPaymentDuration} />
                                            )}
                                        </Grid>
                                    </Grid>
                                    <Grid item xs={2}>
